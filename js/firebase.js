...
async function addReaction(recipeSlug, type) {
  try {
    ...
    const saved = await push(reactionRef, { type, timestamp: Date.now() });
    if (!saved) reactionsCache[recipeSlug].pop();
    return true;
  } catch (e) {
    if (reactionsCache[recipeSlug]) reactionsCache[recipeSlug].pop();
