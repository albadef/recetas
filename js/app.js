...
function renderFilters() {
  ...
  tipoContainer.innerHTML = TIPO_TAGS.map(tag =>
...
function renderRecipes() {
  ...
  grid.innerHTML = filteredRecipes.map(r => {
    ...
    const avgVotes = window.firebaseAPI ? window.firebaseAPI.getCombinedAverage(r.slug, r.valoracion_gastronomica_chef_alba) : r.valoracion_gastronomica_chef_alba;
    const voteCount = window.firebaseAPI ? window.firebaseAPI.getCombinedCount(r.slug, r.valoracion_gastronomica_chef_alba) : 7;
      ...
      <article class="recipe-card" onclick="openModal(${r.id})">
        ...
        <div class="recipe-body">
          ...
          <div class="recipe-meta">
            ...
            <span class="votes-count" title="${t('based_on_votes', voteCount)}">(${voteCount})</span>
          </div>
        </div>
      </article>
    `;
...
function openModal(id) {
  ...
  const userVoted = window.firebaseAPI ? window.firebaseAPI.hasUserVoted(recipe.slug) : null;
  const avgVotes = window.firebaseAPI ? window.firebaseAPI.getCombinedAverage(recipe.slug, recipe.valoracion_gastronomica_chef_alba) : recipe.valoracion_gastronomica_chef_alba;
  const voteCount = window.firebaseAPI ? window.firebaseAPI.getCombinedCount(recipe.slug, recipe.valoracion_gastronomica_chef_alba) : 7;
  const avgRounded = Math.round(avgVotes);
    ...
    <div class="modal-content">
      ...
      <div class="visitors-votes">
        <div class="visitors-votes-header">
          ...
          <div class="visitors-average">
            <span class="visitors-average-num">${avgVotes.toFixed(1)}</span>
            <span class="visitors-stars">${'★'.repeat(avgRounded)}${'<span style="opacity:0.25">★</span>'.repeat(5 - avgRounded)}</span>
          </div>
        </div>
        <p style="font-size:12px;color:var(--accent-dark);opacity:0.7;">${t('based_on_votes', voteCount)}</p>

        <div class="visitors-vote-prompt">
          <span>${userVoted ? t('your_vote_thanks') : t('your_vote')}</span>
          <div class="user-stars" id="user-stars">
            ${[1,2,3,4,5].map(n => `
              <button class="user-star ${userVoted >= n ? 'voted' : ''}" data-value="${n}"
                      onclick="submitVote('${recipe.slug}', ${n})"
                      onmouseenter="hoverStars(${n})"
                      onmouseleave="resetStarHover()"
                      ${userVoted ? 'disabled' : ''}>★</button>
            `).join('')}
          </div>
        </div>
      </div>
...
function hoverStars(n) {
  document.querySelectorAll('#user-stars .user-star').forEach((btn, i) => {
    btn.classList.toggle('hover', i < n);
  });
}

function resetStarHover() {
  document.querySelectorAll('#user-stars .user-star').forEach(btn => {
    btn.classList.remove('hover');
  });
}

async function submitVote(slug, rating) {
  if (!window.firebaseAPI) return;
  if (window.firebaseAPI.hasUserVoted(slug)) return;

  // Save in localStorage
  window.firebaseAPI.setUserVote(slug, rating);

  // Mark stars
  document.querySelectorAll('#user-stars .user-star').forEach((btn, i) => {
    btn.classList.toggle('voted', i < rating);
    btn.disabled = true;
  });

  // Send to Firebase
  await window.firebaseAPI.addVote(slug, rating);

  // Update prompt text
  const prompt = document.querySelector('.visitors-vote-prompt span');
  if (prompt) prompt.textContent = t('your_vote_thanks');

  // Re-render cards to show updated votes
