...
function applyFilters() {
  ...
  filteredRecipes = RECIPES.filter(r => {
    ...
    if (state.especial.size > 0) {
      const hasAll = [...state.especial].every(t => r.etiquetas.includes(t));
...
async function submitReaction(slug, type) {
  ...
  const savePromise = window.firebaseAPI.addReaction(slug, type);
  ...
  const reactions = getReactionSummary(slug);
  const score = document.querySelector('.visitors-average-num');
  const totals = document.querySelector('.reaction-totals');
  if (score) score.textContent = `${reactions.percentLikes}%`;
  if (totals) {
    totals.innerHTML = `
      <span>👍 ${reactions.likes}</span>
      <span>👎 ${reactions.dislikes}</span>
